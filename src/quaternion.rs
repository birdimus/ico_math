...
impl Quaternion{
	...
	pub fn new(x : f32, y : f32, z : f32, w : f32) -> Quaternion{
		unsafe{
			Quaternion{data : _mm_set_ps(w, z, y, x)}
		}
	}
	...
	pub fn angle_axis<T : Into<FloatVector>>(radians : T, axis : Vector3) -> Quaternion{	
		...
		let mut q = Quaternion{data:sin_axis.data};
		q.set_w(cos_angle);
		return q;
	...
	pub fn dot(q1 : Quaternion, q2 : Quaternion) -> FloatVector{	
		unsafe{
			return FloatVector{data : _ico_dp4_ps(q1.data,q2.data)};
		}
	}
	...
	pub fn normalize(q : Quaternion)->Quaternion{
		...
		return Quaternion::from(
			Vector4::select(Vector4::from(Quaternion::identity()), scaled, mask)
			);
		// unsafe{
	 // 		let sqr_length = _ico_dp4_ps(q.data, q.data);
